...
async function getMovieInfo(payload: string, lang: string): Promise<AxiosResponse<GetAdditionalInfoResponse>> {
  return await api.get<GetAdditionalInfoResponse>(`${API_URL}/${lang}/API/Title/${API_KEY}/${payload}/Ratings`);
}

export function* getMovieInfoAsync(action: GetMovieInfo) {
  try {
    const lang = yield select(selectCurrentLanguage);
    const response: AxiosResponse<GetAdditionalInfoResponse> = yield call(getMovieInfo, action.payload, lang);
...
export function* watchGetMovieInfo(axiosApi: AxiosInstance): Generator {
  yield takeLatest<GetMovieInfo>(types.GET_MOVIE_INFO_STARTED, getMovieInfoAsync);
}
