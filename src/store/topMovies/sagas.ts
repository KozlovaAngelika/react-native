...
async function getTopMovies(
  lang: string,
): Promise<AxiosResponse<GetTopMoviesResponse>> {
  return await api.get<GetTopMoviesResponse>(
    `/${lang}/API/Top250Movies/${API_KEY}`,
  );
}
...
export function* getTopMoviesAsync(action: GetTopMovies) {
  try {
    ...
    const response: AxiosResponse<GetTopMoviesResponse> = yield call(
      getTopMovies,
      lang,
    );
...
export function* watchGetTopMovies(axiosApi: AxiosInstance): Generator {
  yield takeEvery<GetTopMovies>(
    types.GET_TOP_MOVIES_STARTED,
    getTopMoviesAsync,
  );
}
