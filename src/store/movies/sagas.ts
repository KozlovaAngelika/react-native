...
async function getMovies(payload: string, lang: string): Promise<AxiosResponse<SearchMovieResponse>> {
  return await api.get<SearchMovieResponse>(`/${lang}/API/SearchMovie/${API_KEY}/${payload}`);
}

export function* searchMoviesAsync(action: SearchMovies) {
  try {
    const lang = yield select(selectCurrentLanguage);
    const response: AxiosResponse<SearchMovieResponse> = yield call(getMovies, action.payload, lang);
...
export function* watchSearchMovies(axiosApi: AxiosInstance): Generator {
  yield takeLatest<SearchMovies>(types.SEARCH_MOVIES_STARTED, searchMoviesAsync);
}
